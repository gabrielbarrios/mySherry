        ...
        <main className="p-6 max-w-7xl mx-auto text-black">
            ...
            {albums?.length === 0 ? (
                <div className="text-center py-20 bg-gray-50 rounded-[3rem] border-4 border-dashed border-gray-100">
                    <p className="text-gray-400 text-xl font-medium">{t.albums.noAlbums}</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {albums?.map((album) => {
                        ...
                        return (
                            <div
                                key={album.id}
                                className="group flex flex-col bg-white border border-gray-100 rounded-[2.5rem] p-2 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 relative"
                            >
                                <Link href={`/albums/view/${album.slug}`} className="flex flex-col flex-grow">
                                    <div className="relative aspect-[16/10] bg-gray-100 rounded-[2rem] mb-4 overflow-hidden flex items-center justify-center text-4xl group-hover:scale-[1.02] transition-transform">
                                        {cover ? (
                                            <img src={cover} alt={album.title} className="w-full h-full object-cover" />
                                        ) : (
                                            <Images size={40} className="text-gray-300" />
                                        )}
                                        <div className="absolute bottom-3 right-3 bg-black/50 text-white text-[10px] font-black px-2.5 py-1 rounded-xl">
                                ...
                                <div className="flex justify-center gap-2 pb-4 pt-2 border-t border-gray-50 mt-auto">
