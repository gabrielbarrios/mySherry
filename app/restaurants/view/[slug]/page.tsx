...
export default async function RestaurantView({ params }: { params: { slug: string } }) {
 ...
 <h2 className="text-2xl font-black text-gray-900 dark:text-gray-100 uppercase tracking-tight">{t.global.aboutUs}</h2>
 </div>
 ...
 <h2 className="text-2xl font-black text-gray-900 dark:text-gray-100 uppercase tracking-tight">{t.global.location}</h2>
 </div>
 <div className="flex flex-col md:flex-row justify-between gap-6 mb-8">
 <div className="space-y-1">
 <p className="text-xl font-bold text-gray-900 dark:text-gray-100">{restaurant.address}</p>
 <p className="text-gray-500 dark:text-gray-400 font-medium">{restaurant.city}, {restaurant.state}</p>
 </div>
 ...
 {restaurant.latitude && (
 <div className="relative overflow-hidden h-72 bg-gray-50 dark:bg-gray-800 rounded-[2rem] border-2 border-dashed border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center group hover:border-blue-300 transition-colors">
 <MapPin size={40} className="text-gray-300 group-hover:text-blue-400 transition-colors mb-2" />
 <span className="text-gray-500 dark:text-gray-400 font-bold text-sm tracking-widest uppercase">{t.global.mapInteractiveSoon}</span>
 </div>
 )}
 </section>
 </div>
 ...
 {restaurant.phone && (
 <a href={`tel:${restaurant.phone}`} className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 dark:bg-gray-800 hover:bg-orange-50 hover:text-orange-600 transition-all border border-transparent hover:border-orange-100 group">
 <div className="bg-white dark:bg-gray-900 p-2 rounded-lg shadow-sm group-hover:text-orange-600">
 <Phone size={20} />
 </div>
 ...
 {restaurant.website && (
 <a href={restaurant.website} target="_blank" className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 dark:bg-gray-800 hover:bg-blue-50 hover:text-blue-600 transition-all border border-transparent hover:border-blue-100 group">
 <div className="bg-white dark:bg-gray-900 p-2 rounded-lg shadow-sm group-hover:text-blue-600">
 <Globe size={20} />
 </div>
 ...
 <span className="text-gray-500 dark:text-gray-400 font-bold uppercase text-[10px] tracking-widest">{daysLabel[day]}</span>
