        ...
        <div className="fixed inset-0 z-110 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-200">
                ...
                <div className="p-4 max-h-[60vh] overflow-y-auto">
                    {loading ? (
                        <p className="text-center py-10 animate-pulse font-bold text-gray-400">{t.global.loading}</p>
                    ) : requests.length === 0 ? (
                        <p className="text-center py-10 text-gray-400">{t.friends.noPendingRequests}</p>
                    ) : (
                        requests.map((req) => (
                            <div key={req.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-3xl mb-3">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => handleResponse(req.id, 'rejected')} className="p-2 bg-gray-200 text-gray-600 rounded-full hover:bg-gray-300 transition-colors">
                                        <X size={18} />
                                    </button>
                                </div>
                            </div>
