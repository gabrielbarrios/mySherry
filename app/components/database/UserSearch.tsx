        ...
        <div className={`relative w-full max-w-md ${className || ''}`}>
            <div className="relative group">
                ...
                <input
                    ...
                    className="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus:ring-2 focus:ring-orange-100 focus:border-orange-500 outline-none transition-all font-medium text-sm"
                />
            ...
            {showResults && results.length > 0 && (
                <>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-3xl shadow-2xl z-20 overflow-hidden animate-in fade-in slide-in-from-top-2">
                        {results.map((user) => (
                            <button
                                key={user.id}
                                onClick={() => handleSelect(user.username)}
                                className="w-full flex items-center gap-3 px-5 py-4 hover:bg-orange-50 transition-colors text-left border-b border-gray-50 last:border-0"
                            >
